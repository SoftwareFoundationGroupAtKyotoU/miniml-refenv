...
let run ?(debug=false) (state: Config.t): Value.t * Store.t =
  let step (state: Config.t): stepResult =
    ...
    | Config.EvalTerm (lv, tm, renv, cenv, cont, store) ->
      if Int.equal lv 0 then
        (match tm with
         (* Already a value *)
         | Term.Int i -> InProgress(Config.ApplyCont(lv, cont, Value.Int i, store))
         | Term.Bool b -> InProgress(Config.ApplyCont(lv, cont, Value.Bool b, store))
         | Term.BinOp (op, arg1, arg2) ->
           InProgress(Config.EvalTerm(lv, arg1, renv, cenv, (Cont.BinOpL0(op, arg2, renv, cenv))::cont, store))
         | Term.UniOp (op, arg) ->
           InProgress(Config.EvalTerm(lv, arg, renv, cenv, (Cont.UniOp0(op)) :: cont, store))
         | Term.ShortCircuitOp (op, arg1, arg2) ->
           InProgress(Config.EvalTerm(lv, arg1, renv, cenv, (Cont.ShortCircuitOpL0(op, arg2, renv, cenv)) :: cont, store))
         | Term.Var var ->
           let v = RuntimeEnv.lookup_var var renv in
           InProgress(Config.ApplyCont(lv, cont, v, store))
         | Term.Lam _ ->
           let v = Value.Clos(renv, cenv, tm) in
           InProgress(Config.ApplyCont(lv, cont, v, store))
         | Term.App (func, arg) ->
           InProgress(Config.EvalTerm(lv, func, renv, cenv, (Cont.AppL0(arg, renv, cenv)) :: cont, store))
         | Term.Quo (cls, body) ->
           InProgress(Config.EvalTerm(lv+1, body, renv, cenv, (Cont.Quof(CodeEnv.rename_cls cls cenv)) :: cont, store))
         | Term.Unq (0, code) ->
           InProgress(Config.EvalTerm(0, code, renv, cenv, (Cont.RuntimeEval0(renv, cenv)) :: cont, store))
         | Term.Unq (_, _) -> failwith "Invalid level given to unquote"
         | Term.PolyCls _ ->
           let v = Value.Clos(renv, cenv, tm) in
           InProgress(Config.ApplyCont(lv, cont, v, store))
         | Term.AppCls (func, cls) ->
           InProgress(Config.EvalTerm(0, func, renv, cenv, (Cont.AppCls0(CodeEnv.rename_cls cls cenv)) :: cont, store))
         | Term.Fix func ->
           InProgress(Config.EvalTerm(0, func, renv, cenv, Cont.Fix0 :: cont, store))
         | Term.If (cond, thenn, elsee) ->
           InProgress(Config.EvalTerm(0, cond, renv, cenv, (Cont.IfCond0(thenn, elsee, renv, cenv)) :: cont, store))
         | Term.Nil -> InProgress(Config.ApplyCont(lv, cont, Value.Nil, store))
         | Term.Ref init ->
           InProgress(Config.EvalTerm(0, init, renv, cenv, Cont.Ref0 :: cont, store))
         | Term.Deref loc ->
           InProgress(Config.EvalTerm(0, loc, renv, cenv, Cont.Deref0 :: cont, store))
         | Term.Assign (dest, newval) ->
           let cont1 = Cont.AssignDest0(newval, renv, cenv) :: cont in
           InProgress(Config.EvalTerm(0, dest, renv, cenv, cont1, store))
         | Term.Letcs (var, ty, cls, tm, body) ->
           let cont1 = Cont.LetcsVal0(var, ty, cls, tm, body, renv, cenv) :: cont in
           InProgress(Config.EvalTerm(lv, tm, renv, cenv, cont1, store))
         | Term.Lift (cls, tm) ->
        ...
        (match tm with
         | Term.Int i ->
           InProgress(Config.ApplyCont(lv, cont, Value.Fut(Term.Int(i)), store))
         | Term.Bool b ->
           InProgress(Config.ApplyCont(lv, cont, Value.Fut(Term.Bool(b)), store))
         | Term.BinOp (op, argl, argr) ->
           let cont1 = Cont.BinOpLf(op, argr, renv, cenv) :: cont in
           InProgress(Config.EvalTerm(lv, argl, renv, cenv, cont1, store))
         | Term.UniOp (op, arg) ->
           let cont1 = Cont.UniOpf(op) :: cont in
           InProgress(Config.EvalTerm(lv, arg, renv, cenv, cont1, store))
         ...
         | Term.Var var ->
           ...
           InProgress(Config.ApplyCont(lv, cont, result, store))
         | Term.Lam (var, ty, cls, body) ->
           ...
           InProgress(Config.EvalTerm(lv, body, renv, cenv1, cont1, store))
         | Term.App (func, arg) ->
           let cont1 = Cont.AppLf(arg, renv, cenv) :: cont in
           InProgress(Config.EvalTerm(lv, func, renv, cenv, cont1, store))
         | Term.Quo (cls, body) ->
           let cont1 = Cont.Quof(CodeEnv.rename_cls cls cenv) :: cont in
           InProgress(Config.EvalTerm(lv + 1, body, renv, cenv, cont1, store))
         | Term.Unq (lvdiff, tm) ->
           if Int.(lv < lvdiff) then
             ...
             InProgress(Config.EvalTerm(lv - lvdiff, tm, renv, cenv, cont1, store))
         ...
         | Term.If (cond, thenn, elsee) ->
           ...
           InProgress(Config.EvalTerm(lv, cond, renv, cenv, cont1, store))
         | Term.Nil ->
           let result = Value.Fut Term.Nil in
           InProgress(Config.ApplyCont(lv, cont, result, store))
         | Term.Ref init ->
           let cont1 = Cont.Reff :: cont in
           InProgress(Config.EvalTerm(lv, init, renv, cenv, cont1, store))
         | Term.Deref loc ->
           let cont1 = Cont.Dereff :: cont in
           InProgress(Config.EvalTerm(lv, loc, renv, cenv, cont1, store))
         | Term.Assign (dest, newval) ->
           let cont1 = Cont.AssignDestf(newval, renv, cenv) :: cont in
           InProgress(Config.EvalTerm(lv, dest, renv, cenv, cont1, store))
         | Term.Letcs (var, ty, cls, def, body) ->
           ...
           InProgress(Config.EvalTerm(lv, def, renv, cenv, cont1, store))
         | Term.Lift (cls, tm) ->
           let cont1 = Cont.Liftf(CodeEnv.rename_cls cls cenv) :: cont in
           InProgress(Config.EvalTerm(lv, tm, renv, cenv, cont1, store))
        )
    | Config.ApplyCont (lv, conts, v, store) ->
      if Int.equal lv 0 then
        (match conts with
         | [] -> Done(v, store)
         | (Cont.BinOpL0(op, tm, renv, cenv) :: rest) ->
           InProgress(Config.EvalTerm(0, tm, renv, cenv, (Cont.BinOpR0(op, v)) :: rest, store))
         | (Cont.BinOpR0(op, v2) :: rest) ->
           let result = Primitives.performBinOp op v2 v in
           InProgress(Config.ApplyCont(0, rest, result, store))
         | (Cont.UniOp0(op) :: rest) ->
         ...
         | (Cont.ShortCircuitOpL0(op, argr, renv, cenv) :: rest) ->
           (match (op, v) with
            | (ShortCircuitOp.And, Value.Bool false) ->
              InProgress(Config.ApplyCont(0, rest, Value.Bool(false), store))
            | (ShortCircuitOp.Or, Value.Bool true) ->
              InProgress(Config.ApplyCont(0, rest, Value.Bool(true), store))
         ...
         | (Cont.AppL0(tm, renv, cenv) :: rest) ->
           InProgress(Config.EvalTerm(0, tm, renv, cenv, (Cont.AppR0 v) :: rest, store))
         ...
         | (Cont.Unq0(lvdiff) :: rest) ->
           (match v with
            | Value.Code (Term.Quo(_, body)) ->
              ...
              InProgress(Config.ApplyCont(0 + lvdiff, rest, result, store))
            | _ -> failwith "expected quoted code"
           )
         ...
         | (Cont.Fix0 :: rest) ->
           (match v with
            | Clos(renv1, cenv1, (Lam(self, _, clss, Lam(v, cls, typ, body)) as fixed)) ->
              ...
              InProgress(Config.ApplyCont(0, rest, (Value.Clos(renv2, cenv1, Lam(v, cls, typ, body))), store))
            | Clos(renv1, cenv1, (Lam(self, _, clss, PolyCls(cls, base, body)) as fixed)) ->
              ...
              let renv2 = (self, clss, eta) :: renv1 in
              InProgress(Config.ApplyCont(0, rest, (Value.Clos(renv2, cenv1, PolyCls(cls, base, body))), store))
         ...
         | Cont.LetcsVal0(var, ty, cls, def, body, renv, cenv) :: rest ->
           ...
           InProgress(Config.EvalTerm(0, body, renv1, cenv, cont1, store))
         | Cont.LetcsBody0(var, ty, cls, def, current_cls) :: rest ->
           (match v with
            ...
            | Value.Bool _
            | Value.Int _ ->
              InProgress(Config.ApplyCont(0, rest, v, store))
         ...
         | Cont.Ref0 :: rest ->
           ...
           InProgress(Config.ApplyCont(0, rest, Value.Loc newloc, (newloc, v) :: store))
         | Cont.Deref0 :: rest ->
           (match v with
            | Value.Loc(loc) ->
              let content = Store.lookup loc store in
              InProgress(Config.ApplyCont(0, rest, content, store))
            | _ -> failwith "expected location")
         | Cont.AssignDest0(newval, renv, cenv) :: rest ->
           let cont1 = Cont.AssignNewVal0 v :: rest in
           InProgress(Config.EvalTerm(0, newval, renv, cenv, cont1, store))
         | Cont.AssignNewVal0(vloc) :: rest ->
           (match (vloc) with
            | (Value.Loc cloc) ->
              let store1 = Store.update cloc v store in
              InProgress(Config.ApplyCont(0, rest, Value.Nil, store1))
            | _ -> failwith "expected location"
           )
        ...
        (match conts with
         | (Cont.BinOpLf(op, tm, renv, cenv) :: rest) ->
           InProgress(Config.EvalTerm(lv, tm, renv, cenv, (Cont.BinOpRf(op, v)) :: rest, store))
         | (Cont.BinOpRf(op, vl) :: rest) ->
           (match (vl, v) with
            | (Value.Fut cl, Value.Fut cr) ->
              let result = Value.Fut(Term.BinOp(op, cl, cr)) in
              InProgress(Config.ApplyCont(lv, rest, result, store))
         ...
         | (Cont.ShortCircuitOpLf(op, argr, renv, cenv) :: rest) ->
           ...
           InProgress(Config.EvalTerm(lv, argr, renv, cenv, cont1, store))
           ...
           (match v with
            | (Value.Fut body) ->
              if Int.equal lv 1 then
                InProgress(Config.ApplyCont(0, rest, Value.Code(Term.Quo(cls, body)), store))
              else
                InProgress(Config.ApplyCont(lv-1, rest, Value.Fut(Term.Quo(cls, body)), store))
            | _ -> failwith "expected future value"
           )
         | (Cont.PolyClsf(cls, base) :: rest) ->
           (match v with
            | (Value.Fut body) ->
              InProgress(Config.ApplyCont(lv, rest, Value.Fut(Term.PolyCls(cls, base, body)), store))
            | _ -> failwith "expected future value"
           )
         | (Cont.AppClsf(cls) :: rest) ->
           (match v with
            | (Value.Fut func) ->
              InProgress(Config.ApplyCont(lv, rest, Value.Fut(Term.AppCls(func, cls)), store))
            | _ -> failwith "expected future value")
         | Cont.Fixf :: rest ->
           (match v with
            | (Value.Fut func) ->
              InProgress(Config.ApplyCont(lv, rest, Value.Fut(Term.Fix func), store))
            | _ -> failwith "expected future value"
           )
         | Cont.IfCondf(thenn, elsee, renv, cenv) :: rest ->
           let cont = Cont.IfThenf(v, elsee, renv, cenv) :: rest in
           InProgress(Config.EvalTerm(lv, thenn, renv, cenv, cont, store))
         | Cont.IfThenf(condv, elsee, renv, cenv) :: rest ->
           let cont = Cont.IfElsef(condv, v) :: rest in
           InProgress(Config.EvalTerm(lv, elsee, renv, cenv, cont, store))
         ...
         | Cont.LetcsValf(var, ty, cls, body, renv, cenv) :: rest ->
           ...
           InProgress(Config.EvalTerm(lv, body, renv, cenv, cont1, store))
         ...
         | Cont.AssignDestf(newval, renv, cenv) :: rest ->
           ...
           InProgress(Config.EvalTerm(lv, newval, renv, cenv, cont1, store))
